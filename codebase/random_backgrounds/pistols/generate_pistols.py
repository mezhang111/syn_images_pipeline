...
def generate_sample_pose_fn(config, surface=None, face_sample_range=[0, 0.5], min_height=0, max_height=0.1):
...
def sample_camera(_poi, config, surface=None, face_sample_range=[0.55, 1.0], min_height=2.5, max_height=4):
    location_cfg = config.location.random
...
def main():
    ...
    p_select = min(args.average_object_per_image*0.7 / len(objs), 1.)
    p_select_other = min(args.average_object_per_image*0.7 / len(other_objs), 1.) if len(other_objs) > 0 else 0
