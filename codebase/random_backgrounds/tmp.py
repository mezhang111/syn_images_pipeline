...
def generate_sample_pose_fn(config, surface=None, face_sample_range=[0, 0.7], min_height=0, max_height=0.01):
...
def sample_camera(_poi, config, surface=None, face_sample_range=[0.6, 0.9], min_height=0.5, max_height=2):
    location_cfg = config.location.random
    ...
    range_min, range_max = utils.get_xyz_range(look_at_cfg, [[-1, 1], [-1, 1], [0.3, 0.7]], [[0, 0]] * 3)
...
def main():
    ...
    utils.preprocess_and_scale_objs(objs, objs_names, bbox_size=config.obj_size if 'obj_size' in config else 1.)
    if args.other_obj is not None:
        ...
        utils.preprocess_and_scale_objs(other_objs, other_objs_names, category_id=0,
                                        bbox_size=config.other_obj_size if 'other_obj_size' in config else 1.)
    else:
        other_objs = []
    ...
    p_select_other = min(1.5 / len(other_objs), 1.) if len(other_objs) > 0 else 0
    ...
    for _ in range(config.num_pose):
        ...
        utils.random_material_texture_infusion(materials, objs+other_objs+lights, texture)
        selected_objs = utils.random_select_obj(objs, p_select=p_select, p_not_select=max(0., 1. - p_select),
                                                max_objs=round(args.average_object_per_image * 1.1))
        ...
        selected_lights = utils.random_select_obj(lights, p_select=p_select_light,
                                                  ...
                                                  max_objs=3)
        n_cameras = 0
        selected_objs_all = selected_objs + selected_objs_other  # put those 2 together because they would be on the ground
        if len(selected_objs_all) > 0:
            if len(selected_objs) > 0:
                ...
                bproc.object.sample_poses(
                    selected_objs,
                    sample_pose_func=generate_sample_pose_fn(config.object, surface=surface, face_sample_range=[0, 0.3]),
                    objects_to_check_collisions=[],
                    max_tries=10
                )
            if len(selected_objs_other) > 0:
                utils.random_material_texture_infusion(materials, selected_objs_other, texture)
                if config.sample_material:
                    utils.sample_material(selected_objs_other)
                # Sample the poses of all shapenet objects above the ground without any collisions in-between
                bproc.object.sample_poses(
                    selected_objs_other,
                    sample_pose_func=generate_sample_pose_fn(config.object, surface=surface,
                                                             face_sample_range=[0.3, 0.5]),
                    objects_to_check_collisions=[]
                )
            ...
            poi = bproc.object.compute_poi(selected_objs_all)
            max_tries = 10000
            n_tries = 0
    ...
    if n_cameras > 0:
        ...
        if store_metadata:
            if not os.path.exists(instance_path) or not os.path.exists(depth_path):
                ...
                for i in range(n_cameras):
                    names = [instance['name'] for instance in data["instance_attribute_maps"][i][1:] if
                             instance['category_id'] > 0]
                    ids = [instance['idx'] for instance in data["instance_attribute_maps"][i][1:] if
                           instance['category_id'] > 0]
